...
describe PostCodesio do
	...
	context "when using multiple postocdes" do


		before(:all) do
			@postcodes_getter = PostCodesio.new
			@postocde_hashes = @postcodes_getter.mutiple_postcodes(["EN93SD","UB81PD","E26dt"])

			@postcode_hash_1 = @postocde_hashes["result"][0]
			@postcode_hash_2 = @postocde_hashes["result"][1]
			@postcode_hash_3 = @postocde_hashes["result"][2]
		end

		it "should return a key called status that returns an Integer" do 

			expect(@postocde_hashes["status"].class).to eq Integer
		...
		it "should include a key caled 'longitude' in result hash that returns a Float beween 90 and -90 inclusive" do
			...
			expect(@postcode_hash_1["result"]["longitude"]).to be <= 90
			expect(@postcode_hash_1["result"]["longitude"]).to be >= -90

			expect(@postcode_hash_2["result"]["longitude"]).to be <= 90
			expect(@postcode_hash_2["result"]["longitude"]).to be >= -90

			expect(@postcode_hash_3["result"]["longitude"]).to be <= 90
			expect(@postcode_hash_3["result"]["longitude"]).to be >= -90
		...
		it "should include a key caled 'latitude' in result hash that returns a Float beween 90 and -90 inclusive" do

			expect(@postcode_hash_1["result"]["latitude"]).to be <= 180
			expect(@postcode_hash_1["result"]["latitude"]).to be >= -180

			expect(@postcode_hash_2["result"]["latitude"]).to be <= 180
			expect(@postcode_hash_2["result"]["latitude"]).to be >= -180

			expect(@postcode_hash_3["result"]["latitude"]).to be <= 180
			expect(@postcode_hash_3["result"]["latitude"]).to be >= -180
